...
static const char *TAG_SERVO_TASK = "servo_task";

#define PROFILE_NUM 1
#define PROFILE_APP_IDX 0
#define ESP_APP_ID 0x55
#define SAMPLE_DEVICE_NAME "ESP_GATTS_ALEX"
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_WRITE_EVT:
        ...
        {
            // Log the task in which this event is running
            ...
            char *hex_str = malloc(3 * param->write.len + 1);
            if (hex_str == NULL)
            {
                ESP_LOGE(GATTS_TABLE_TAG, "Failed to allocate memory for HEX string");
                return;
            }
            ...
            {
                snprintf(&hex_str[i * 3], 4, "%02X ", param->write.value[i]);
            }

            // the data length of gattc write  must be less than GATTS_DEMO_CHAR_VAL_LEN_MAX.
            ESP_LOGI(GATTS_TABLE_TAG, "-----------------------------------------");
            ESP_LOGI(GATTS_TABLE_TAG, "BLE Write Event Received");
            ESP_LOGI(GATTS_TABLE_TAG, "Handle: %d | Value Length: %d | Value (HEX): %s",
                     param->write.handle, param->write.len, hex_str);

            free(hex_str); // Free allocated memory

            if (param->write.len > 0)
            {
                char angle_str[10] = {0}; // Buffer to hold the received angle string
                int received_angle;

                // Convert received BLE data (ASCII) to an integer
                memcpy(angle_str, param->write.value, param->write.len);
                angle_str[param->write.len] = '\0'; // Null-terminate the string
                received_angle = atoi(angle_str);   // Convert to integer

                // Clamp angle within valid servo range (-90 to 90 degrees)
                if (received_angle < -90)
                    received_angle = -90;
                if (received_angle > 90)
                    received_angle = 90;

                // Send angle value to the queue
                if (xQueueSend(servo_queue, &received_angle, portMAX_DELAY) != pdPASS)
                {
                    ESP_LOGE(GATTS_TABLE_TAG, "Failed to send angle to queue");
                }
                else
                {
                    ESP_LOGI(GATTS_TABLE_TAG, "Sent angle %d to queue", received_angle);
                }
            }
...
{
    int received_angle;
    int current_angle = 0; // Track the last applied angle

    while (1)
    {
        // Wait for a new angle value from BLE
        if (xQueueReceive(servo_queue, &received_angle, portMAX_DELAY))
        {
            ESP_LOGI(TAG_SERVO_TASK, "-----------------------------------------");
            ESP_LOGI(TAG_SERVO_TASK, "Received new angle: %d°", received_angle);
            ESP_LOGI(TAG_SERVO_TASK, "Previous servo angle: %d°", current_angle);

            // Update servo to new angle
            set_angle(received_angle);

            // Update current angle after setting
...
void app_main(void)
{
    ...
    ESP_LOGI(GATTS_TABLE_TAG, "Starting BLE advertising...");
    esp_ble_gap_start_advertising(&adv_params);

    // Initialize the servo motor
    ESP_LOGI("Main", "Initializing servo control...");
    init_servo();
    ...
    // Start a separate task for the servo control loop
    xTaskCreate(servo_task, "servo_task", 2048, NULL, 5, NULL);

    // Debugging: Print all running tasks
    char *task_list_buffer = pvPortMalloc(1024);
    if (task_list_buffer)
    {
        vTaskList(task_list_buffer);
        ESP_LOGI("TASK_LIST", "\nTask Name       State  Prio  Stack  Task ID%s", task_list_buffer);
        vPortFree(task_list_buffer);
    }

    ESP_LOGI("Main", "Setup complete. Waiting for BLE connections...");
}
